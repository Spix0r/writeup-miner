...
def setup_argparse():
    ...
    parser.add_argument("-c", "--chatid", help="Telegram chatid", required=True)
...
def main():
    ...
    if args.dbmode == "file":
        ...
        if os.path.exists(FILEDBDIR):
            filedb.checkDatabase(newFeeds, FILEDBDIR, args.token, args.chatid)
        ...
        if is_exist:
            mongodb.check_database(mydb, newFeeds, args.token, args.chatid)
        else:
            mongodb.updateDatabase(mydb, newFeeds, exist=is_exist)
